...
export async function startDFSWorker(dfsId) {
  ...
  async function scrape() {
    // If we've seen no table several times, stop hammering this game
    if (consecutiveEmpty >= MAX_EMPTY) {
      console.log(
        `DFS worker ${dfsId}: stopping after ${MAX_EMPTY} empty polls (no stats table)`
      );
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        console.log(
          `DFS worker ${dfsId}: HTTP ${res.status} from DFS Australia`
        );
        return;
      }
      ...
      if (!statsTable) {
        consecutiveEmpty++;
        console.log(
          `DFS worker ${dfsId}: no stats table found (empty #${consecutiveEmpty})`
        );
        return;
      }
  ...
  // Initial scrape (non-blocking for the rest of the app)
  scrape().catch((err) =>
